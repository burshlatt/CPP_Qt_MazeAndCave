...
void MazeWidget::SaveFile(const QString &path) {
    QFile file(path);
...
void MazeWidget::SetWalls(int rows, int cols) {
  ...
  v_walls_ = maze_controller_->GetVerticalWalls();
  h_walls_ = maze_controller_->GetHorizontalWalls();
}
...
void MazeWidget::mousePressEvent(QMouseEvent *event) {
    ...
    if (dots_.size() == 2) {
        way_ = maze_controller_->MakeWay(rows_, cols_, v_walls_, h_walls_, coords_[0], coords_[1]);
        DrawWay();
    }
}
}  // namespace s21
